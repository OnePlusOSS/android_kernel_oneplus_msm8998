...
struct page *(*ctech_defrag_alloc_t)(struct zone *zone, unsigned long *flags, int migratetype, int order, gfp_t gfp_flags) = NULL;
long (*ctech_defrag_calc_t)(struct zone *zone, int order, int alloc_flag) = NULL;
void __mod_zone_page_state(struct zone *, enum zone_stat_item item, long);

/* calling functions */
struct page *ctech_defrag_alloc(struct zone *zone, unsigned long *flags, int migratetype, int order, gfp_t gfp_flags)
{
	if (ctech_defrag_alloc_t)
		return ctech_defrag_alloc_t(zone, flags, migratetype, order, gfp_flags);
	return 0;
}
...
/* register functions */
void ctech_defrag_alloc_reg(struct page *(*fn)(struct zone *zone, unsigned long *flags, int migratetype, int order, gfp_t gfp_flags))
{
