...
static void switch_dev_work(struct work_struct *work)
{
	...
	for (i = 0; i < 3; i++) {
		/*if gpio status is the same as before,ignore them*/
		if (key_pre[i] == key[i])
			state_same++;
		if (state_same == 3)
			return;
	...
	for (i = 0; i < 3; i++) {
	    /*1,if the gpio key stay low ,the gpio is in wrong states*/
		if (key[i] + key_pre[i] == 0) {
			pr_err("[sk]key[%d] is something wrong state\n", i);
			have_wrong_key = true;
			delay_time = 300;
			break;
		}
...
static int tristate_dev_probe(struct platform_device *pdev)
{
	...
	ret = gpio_request(switch_data->key2_gpio, "tristate_key2");
	if (ret < 0)
		goto err_request_gpio;
	...
	ret = gpio_request(switch_data->key3_gpio, "tristate_key3");
	if (ret < 0)
		goto err_request_gpio;
